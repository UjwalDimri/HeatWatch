/**
 * calculations/utci.js
 *
 * Universal Thermal Climate Index (UTCI) — 6th-order polynomial approximation.
 *
 * PROVENANCE / VERSION PINNING:
 *   Coefficients ported verbatim (unchanged) from the validated implementation
 *   `_utci_optimized` in pythermalcomfort (CenterForTheBuiltEnvironment/
 *   pythermalcomfort, pythermalcomfort/models/utci.py, fetched 2026-09-02),
 *   which is itself a port of the official UTCI_a002 Fortran regression
 *   function (Bröde et al., utci.org). Tested against reference values in
 *   tests/utci.test.js.
 *
 * INPUT CONVENTIONS (verified against the reference implementation):
 *   airTemperatureC    dry-bulb air temperature, °C
 *   tmrtC              mean radiant temperature, °C
 *   windSpeed10mMs     wind speed at 10 m above ground, m/s
 *   vapourPressureKpa  water vapour pressure, kPa (polynomial variable `pa` is in kPa)
 *
 * APPLICABILITY LIMITS (official model domain):
 *   -50 ≤ Ta ≤ 50 °C ; -30 ≤ (Tmrt − Ta) ≤ 70 °C ; 0.5 ≤ v ≤ 17 m/s.
 *   Wind below 0.5 m/s is clamped to 0.5 m/s (documented convention of the
 *   reference model). Other out-of-domain inputs → status "unavailable".
 *
 * OUTPUT: UTCI equivalent temperature, °C.
 *
 * UTCI is a deterministic scientific calculation. It is never produced by, or
...
function utciPolynomial(tdb, v, delta_t_tr, pa) {
  return (
    tdb
    + 0.607562052
    + (-0.0227712343) * tdb
 ...
 * @param {number} p.tmrtC
 * @param {number} p.windSpeed10mMs
 * @param {number} p.vapourPressureKpa
 * @returns {{utciC: (number|null), status: string, reason?: string}}
 */
function calculateUtci({ airTemperatureC, tmrtC, windSpeed10mMs, vapourPressureKpa }) {
  const nums = [airTemperatureC, tmrtC, windSpeed10mMs, vapourPressureKpa];
  if (nums.some((n) => typeof n !== 'number' || !Number.isFinite(n))) {
    return {
      utciC: null,
      status: 'unavailable',
      reason: 'Missing or non-numeric UTCI input (Ta, Tmrt, wind or vapour pressure).',
    };
  }

  const ta = airTemperatureC;
  const dTmrt = tmrtC - ta;
  let v = windSpeed10mMs;
  if (v < 0) {
    return { utciC: null, status: 'unavailable', reason: 'Negative wind speed.' };
  }
  if (v < 0.5) v = 0.5; // official low-wind convention of the reference model

  if (ta < -50 || ta > 50) {
    return { utciC: null, status: 'unavailable', reason: 'Air temperature outside UTCI model domain (−50 to 50 °C).' };
  }
  if (dTmrt < -30 || dTmrt > 70) {
    return { utciC: null, status: 'unavailable', reason: 'Tmrt − Ta outside UTCI model domain (−30 to +70 °C).' };
  }
  if (v > 17) {
    return { utciC: null, status: 'unavailable', reason: 'Wind speed outside UTCI model domain (0.5 to 17 m/s).' };
  }
  if (vapourPressureKpa < 0 || vapourPressureKpa > 5) {
    return { utciC: null, status: 'unavailable', reason: 'Vapour pressure outside plausible range (0–5 kPa).' };
  }

  const value = utciPolynomial(ta, v, dTmrt, vapourPressureKpa);
  if (!Number.isFinite(value)) {
    return { utciC: null, status: 'unavailable', reason: 'UTCI polynomial produced a non-finite value.' };
  }
  return { utciC: Math.round(value * 10) / 10, status: 'calculated' };
}

module.exports = { calculateUtci, _utciPolynomial: utciPolynomial };
